...
class App_CajaDeSeguridad(QApplication):
    ...
    def mostrar_elemento(self, id_elemento=-1):
        ...
        if id_elemento != -1:
            elemento = self.logica.dar_elementos()[id_elemento]
            if elemento.tipo == TipoElemento.LOGIN.value:
                self.mostrar_login(elemento.id)
            elif elemento.tipo == TipoElemento.IDENTIFICACION.value:
                self.mostrar_id(elemento.id)
            elif elemento.tipo == TipoElemento.TARJETA.value:
                self.mostrar_tarjeta(elemento.id)
            elif elemento.tipo == TipoElemento.SECRETO.value:
                self.mostrar_secreto(elemento.id)
            else:
                print("Error tipo")
    ...
    def mostrar_clave(self, ventana, id_elemento):
        ...
        """
        self.elemento_actual = self.logica.dar_elementos()[id_elemento]
        QMessageBox.information(ventana, 'Clave elemento',
                                "Nombre clave favorita: " + self.elemento_actual.clave.nombre + "\nClave: " +
                                self.elemento_actual.clave.clave,
                                QMessageBox.Ok)
