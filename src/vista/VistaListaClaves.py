...
class VistaListaClaves(QWidget):
    ...
    def mostrar_dialogo_editar_clave(self, id_clave):
        ...
        """
        clave = self.claves[id_clave]
        dialogo=VistaCrearClave(clave, self.interfaz)
        dialogo.exec_()
        if dialogo.resultado==1:            
            self.interfaz.editar_clave(clave.id, dialogo.texto_nombre.text(), dialogo.texto_clave.text(),dialogo.texto_pista.text())
